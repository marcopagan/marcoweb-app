<script lang="ts">
	...
	import { STEP, snap, getCoordinates, convertCoordinates } from '$lib/rectolib';
	...
	function handleMove(event: MouseEvent | TouchEvent) {
		if (isDraggable) {
			const { x, y } = getCoordinates(event);
			m2 = { x: x, y: y };
			tempRecto = convertCoordinates(m1.x, m1.y, m2.x, m2.y, options.snapToGrid);
   ...
   function updateTheme(colorScheme: ColorScheme){
		for (const [key, value] of Object.entries(colorScheme)) {
			if (key == 'isDark'){
				if (value){document.documentElement.style.setProperty('--gridTheme', `url(gridDark.svg)`) }
				else{document.documentElement.style.setProperty('--gridTheme', `url(gridLight.svg)`) }
			}
...
{#if optionsLoaded}
<div id="background" class={options.showGrid ? 'bgrid' : ''}></div>
	...
	{#each permarectos as recto}
		{#if recto.content === 'BUTTON' && recto.visibility}
			...
			>
				<h3>GO TO</h3>
				<p>
					<button class="disabled-btn">Selected projects<br><span>(coming soon)</span></button>
...
<style>
	...
	.disabled-btn{
		text-align: left;
		line-height: 1.15;
		span{font-family: 'APL386', monospace; text-transform: uppercase; font-size: 1rem; letter-spacing: -.05rem;}
